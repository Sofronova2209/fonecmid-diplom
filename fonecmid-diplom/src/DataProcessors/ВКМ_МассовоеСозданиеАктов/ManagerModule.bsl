...
Функция СозданиеСпискаНаСервере(Дата,СоздатьОбъект) Экспорт
	...
	|ГДЕ
	|	&Дата
	|		МЕЖДУ ДоговорыКонтрагентов.ВКМ_ДатаНачалаДействияДоговора И ДоговорыКонтрагентов.ВКМ_ДатаОкончанияДействияДогвора
	...
	Пока Результат.Следующий() Цикл
		...
		ДокументРеализация = Результат.Реализация;
		Если ЗначениеЗаполнено(ДокументРеализация) Тогда
			ДокОбъект = Результат.Реализация.ПолучитьОбъект();
		    ДокОбъект.ВыполнитьАвтозаполнение();
		    ДокОбъект.Записать(РежимЗаписиДокумента.Проведение);
		Иначе
			ДокументРеализация = СозданиеРеализации(Результат, КонецМесяца(Дата));
		КонецЕсли;
		...
		//КонецЕсли;
		РеализацииСтруктура.Вставить("Договор",Результат.Договор);
	    РеализацииСтруктура.Вставить("Реализация", ДокументРеализация);
...
Если НовыйДок.ПроверитьЗаполнение() Тогда
	...
	Исключение
		ОтменитьТранзакцию();
		ОбщегоНазначения.СообщитьПользователю("Проведение Документа не возможно");
